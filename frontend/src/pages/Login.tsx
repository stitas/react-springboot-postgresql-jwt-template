...
export default function LoginPage() {
  ...
  function validate() {
    ...
    if (!form.password) next.password = "Įveskite slaptažodį";
    setErrors(next);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      setSubmitting(true);
      response = await apiClient.post("/v1/auth/login", {
        json: payload,
      ...
      if (err instanceof ApiError) {
        switch (err.response.status) {
          case 401:
            toast.warning(getErrorMessage("wrongCredentials"));
            break;
          default:
    ...
    <div className="min-h-screen grid place-items-center bg-[var(--color-muted)] px-4 py-10">
      <Card className="w-full max-w-md rounded-2xl shadow-lg">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-semibold">Prisijungti</CardTitle>
          <CardDescription>Prisijunkite el. paštu ir slaptažodžiu arba tęskite su Google.</CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={onSubmit}>
            ...
            <div className="grid gap-2">
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/3 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input
                  id="password"
                ...
                <div className="flex items-center justify-end text-sm text-muted-foreground">
                  <a className="text-[var(--color-primary)] hover:underline" href="#" onClick={() => onPasswordReset()}>Užmiršote slaptažodį ?</a>
                </div>
                <button
                  type="button"
                  aria-label={showPassword ? "Slėpti slaptažodį" : "Rodyti slaptažodį"}
                  onClick={() => setShowPassword((v) => !v)}
                  className="absolute right-0 top-1/4 grid place-items-center px-3 text-muted-foreground hover:text-foreground"
                >
        ...
        <CardFooter className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          <a className="text-[var(--color-primary)] hover:underline" href="/sign-up">Sukurkite paskyrą</a>
        </CardFooter>
      </Card>
